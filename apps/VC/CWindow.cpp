...
void CWindow::OnPathCustomContextMenu(const QPoint& point)
{
    ...
    if (index.isValid()) {
        ...
        QAction* actInkDetect = new QAction(tr("Run vc_layers & Ink Detection"), this);
        connect(actInkDetect, &QAction::triggered, this, [segID, this](){ OnRunInkDetection(segID); });
...
{
    auto folder = fVpkg->segmentation(segmentID.toStdString())->path().native();
    // if (!fs::exists(folder)) {
    ...
    arguments << "-v" << fVpkgPath << "-s" << segmentID << "-o" << QString("%1/%2.obj").arg(QString::fromStdString(folder)).arg(segmentID)
        << "--uv-plot" << QString("%1/uv_%2.tif").arg(QString::fromStdString(folder)).arg(segmentID) << "--mesh-resample-smoothing" << "3"
        << "--output-ppm" << QString("%1/%2.ppm").arg(QString::fromStdString(folder)).arg(segmentID);
    ...
    connect(myProcess, &QProcess::finished, this, [this, segmentID, name](){
        std::cout << "Finished: " << segmentID.toStdString() << ": " << name.toStdString() << std::endl;
        auto vc_layers = new QProcess(this);
        QStringList arguments;
        arguments << "-p"  << QString("1.ppm").arg(segmentID) << "--output-dir" << "layers/";
        vc_layers->start("vc_layers_from_ppm", arguments);
    });
    connect(myProcess, &QProcess::errorOccurred, this, [this, segmentID, name](){ std::cout << "Error: " << segmentID.toStdString() << ": " << name.toStdString() << std::endl; });
...
void CWindow::OnRunInkDetection(QString segmentID) {
    ///// Resample the segmentation /////
    // Mesh the point cloud
    vc::meshing::OrderedPointSetMesher mesher;
    mesher.setPointSet(fSegStructMap[segmentID.toStdString()].fMasterCloud);
    ...
    auto folder = fVpkg->segmentation(segmentID.toStdString())->path().native() + "/layers";
    if (!fs::exists(folder)) {
        fs::create_directory(folder);
    }
    auto outputPath = fs::canonical(folder);
    auto outputPathMask = fs::canonical(fVpkg->segmentation(segmentID.toStdString())->path().native() + "/" + segmentID.toStdString() + "_mask.png");
    ...
    auto imageViewer = new CVolumeViewerWithCurve(fSegStructMap);
    ...
    auto aImgQImage = Mat2QImage(aImgMat);
    imageViewer->SetImage(aImgQImage);

    auto wnd = new QMainWindow(this);
    wnd->setCentralWidget(imageViewer);
    wnd->show();
}
